...
// Fee model: first bin is FREE; charge starts from 2nd bin onward.
// fee = (binCount - 1) × extraBinFeePaise for binCount >= 2, else 0.
describe('requiresExtraBin', () => {
  it('2 meals → 1 bin, ₹0 fee (first bin free)', () => {
    expect(requiresExtraBin(2)).toEqual({ required: false, binCount: 1, extraFeePaise: 0 });
  });

  it('3 meals → 2 bins, ₹2 fee (extra bin)', () => {
    expect(requiresExtraBin(3)).toEqual({ required: true, binCount: 2, extraFeePaise: 200 });
  });

  it('5 meals → 3 bins, ₹4 fee (2 extra bins)', () => {
...
describe('assignBins', () => {
  // ── Meals only ────────────────────────────────────────────────────────────
  it('1 meal → 1 bin, ₹0 fee (first bin free)', () => {
