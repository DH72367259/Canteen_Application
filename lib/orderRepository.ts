...
export async function listOrdersForUser(uid: string): Promise<CanteenOrder[]> {
  ...
  const projections = [
    "*, order_items(*, menu_items(name)), profiles!user_id(name), canteens(name), order_bins(bin_index, bin_code, bin_color, items)",
    "*, order_items(*, menu_items(name)), profiles!user_id(name), canteens(name)",
...
export async function listRecentOrders(limitCount = 100, canteenId?: string): Promise<CanteenOrder[]> {
  ...
  const projections = [
    "*, order_items(*, menu_items(name)), profiles!user_id(name), canteens(name), bins!orders_bin_id_fkey(id, bin_code, color), time_slots(slot_name, start_time, end_time), order_bins(bin_index, bin_code, bin_color, items)",
    "*, order_items(*, menu_items(name)), profiles!user_id(name), canteens(name), order_bins(bin_index, bin_code, bin_color, items)",
    "*, order_items(*, menu_items(name)), profiles!user_id(name), canteens(name)",
    "*, order_items(*, menu_items(name))",
  ];
...
export async function updateOrderStatus(id: string, status: OrderStatus): Promise<CanteenOrder | null> {
  ...
  const { data: row, error } = await supabase
    ...
    .update({ status })
    .eq("id", id)
    .select("*, order_items(*, menu_items(name)), profiles!user_id(name)")
    .single();
