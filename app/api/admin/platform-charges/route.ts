...
export async function PATCH(request: Request) {
  ...
  const { charge_pct, flat_charge, gst_pct, extra_bin_fee_paise } = body;
  ...
  if (extra_bin_fee_paise !== undefined && (Number(extra_bin_fee_paise) < 0))
    return Response.json({ error: "extra_bin_fee_paise must be >= 0." }, { status: 400 });
  ...
  if (extra_bin_fee_paise !== undefined) updates.extra_bin_fee_paise = Math.round(Number(extra_bin_fee_paise));

  let data, error;
  if (id) {
    ({ data, error } = await supabase.from("platform_charges").update(updates).eq("id", id).select("*").single());
  } else {
    ({ data, error } = await supabase.from("platform_charges").insert({
      charge_pct: Number(charge_pct ?? 2),
      flat_charge: Number(flat_charge ?? 0),
      gst_pct: Number(gst_pct ?? 18),
      extra_bin_fee_paise: Math.round(Number(extra_bin_fee_paise ?? 200)),
      updated_by: ctx.uid,
    }).select("*").single());
  }
