/**
 * POST /api/auth/sms-otp/send
 *
 * Sends a 6-digit OTP to an Indian mobile number via Fast2SMS (~₹0.15/SMS).
 * Replaces Twilio Verify entirely.
 *
 * Rate limits (Redis-backed, fail-open):
 *   - 5 OTPs per phone per hour
 *   - 10 OTPs per phone per day
 *
 * Body:   { phone: string }  — accepts +91XXXXXXXXXX or 10-digit format
...
async function checkRateLimit(phone: string): Promise<boolean> {
  ...
  try {
    ...
    const hKey = `sms:h:${phone}:${Math.floor(now / 3600)}`;
    const dKey = `sms:d:${phone}:${Math.floor(now / 86400)}`;
    const [hc, dc] = await Promise.all([redis.get(hKey), redis.get(dKey)]);
    if (Number(hc ?? 0) >= 5 || Number(dc ?? 0) >= 10) return false;
    await Promise.all([
