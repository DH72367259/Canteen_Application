...
function CartContent() {
  ...
  // Parse cart from query string: ?cart=id:name:price:qty,...
  const [cart, setCart] = useState<CartItem[]>(() => {
    const raw = params.get("cart");
    if (!raw) return [];
    return raw.split(",").map((chunk) => {
      const [id, name, price, qty] = chunk.split(":");
      return { id, name, price: Number(price), qty: Number(qty) };
    });
  });

  const [slot, setSlot]         = useState<string | null>(null);
  const [walletBal]             = useState(12); // TODO: fetch from backend
  ...
  async function finaliseOrder(paymentId: string) {
    try {
      // Generate OTP and bin server-side in production via /api/orders
      // Demo: generate locally
      const otp = String(Math.floor(100000 + Math.random() * 900000));
      const bin = `Bin #${Math.floor(Math.random() * 10) + 1}`;
      const orderId = `ORD-${Date.now().toString(36).toUpperCase()}`;
      console.info("Order placed", { orderId, paymentId, otp }); // server log only
      setPlacedOrder({ id: orderId, bin, otp });
    } catch {
  ...
  if (placedOrder) {
    return (
      <div className="app-shell" style={{ alignItems: "center", justifyContent: "center", textAlign: "center", padding: "2rem 1rem" }}>
        ...
        <p style={{ color: "var(--ink-3)", marginBottom: "1.5rem", fontSize: "0.9rem" }}>Your order <strong>{placedOrder.id}</strong> is being prepared.</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", width: "100%", maxWidth: 320 }}>
          <button className="btn btn-primary btn-full" onClick={() => router.push("/dashboard/orders")}>View My Orders</button>
  ...
  return (
    <div className="app-shell">
      <div className="topbar">
        ...
        <h1 style={{ fontSize: "1rem", fontWeight: 700 }}>Checkout</h1>
        <div />
      </div>
        ...
        <section>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
            {SLOTS.map(s => (
              ...
              >
                {s.label}
                {!s.available && <span style={{ fontSize: "0.7rem", marginLeft: 4 }}>Full</span>}
              </button>
            ))}
          </div>
        </section>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, padding: "0.75rem 1rem", background: "var(--surface)", borderTop: "1px solid var(--border)", zIndex: 35 }}>
        ...
        >
          {busy ? "Processing…" : payable === 0 ? "Place Order (Wallet)" : `Pay ₹${payable} →`}
        </button>
      </div>
    </div>
  );
